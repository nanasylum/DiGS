...
class Deformation(nn.Module):
    def __init__(self, D=8, W=256, input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
        ...
        if self.args.empty_voxel:#false
            self.empty_voxel = DenseGrid(channels=1, world_size=[64,64,64])
        if self.args.static_mlp: #false
    ...
    def forward_dynamic(self, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        elif self.args.empty_voxel:#false
        ...
        if self.args.no_dr :#false
            ...
            if self.args.apply_rotation:#false
...
def smooth_pcd(xyz):
    # 先save成ply，再smooth
    # print("[DEBUG] smoothing...")
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(xyz.cpu().detach().numpy())

    # denoise
    denoised_pcd = pcd.voxel_down_sample(voxel_size=0.05)
    # denoised_cloud, _ = o3d.geometry.PointCloud.remove_statistical_outlier(denoised_cloud, num_neighbors=20, std_ratio=2.0)
    denoised_pcd.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)
    # smooth
    # smoothed_pcd = denoised_pcd.smooth_laplacian(alpha=0.99) 

    # gaussian_filter
    # gaussian_filter = o3d.geometry.PointCloud.make_gaussian_filter(sigma=0.5)
    # filtered_pcd = gaussian_filter.filter(pcd)
    # filtered_pcd = pcd.gaussian_filter(pcd, sigma=0.5)

    # xyz_np = np.asarray(pcd.points, dtype='float32') 
    xyz = torch.tensor((np.asarray(denoised_pcd.points, dtype='float32')) , device=xyz.device)
    return xyz