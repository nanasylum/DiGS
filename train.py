...
def scene_reconstruction(mp, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    bg_color = [1, 1, 1] if mp.white_background else [0, 0, 0] #true
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        for viewpoint_cam in viewpoint_cams:
            ...
            # print("[DEBUG] AST...iteration==",iteration)
            time_interval = 1 / 156
            smooth_term = get_linear_noise_func(lr_init=0.1, lr_final=1e-15, lr_delay_mult=0.01, max_steps=20000)
            
            ast_noise = torch.randn(1, 1, device='cuda').expand(gaussians.get_xyz.shape[0], -1)*time_interval*smooth_term(iteration)
            # print("[DEBUG] ast_noise==", ast_noise[0])
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, ast_noise, stage=stage, \
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), \
                testing_iterations, scene, render, [pipe, background], stage, ast_noise, scene.dataset_type, multi_scale)
                ...
                if (iteration < 1000 and iteration % 10 == 9) \
                    or (iteration < 3000 and iteration % 50 == 49) \
                        ...
                        render_training_image(scene, gaussians, [test_cams[iteration%len(test_cams)]], \
                            render, pipe, background, ast_noise, stage+"test", iteration,timer.get_elapsed_time(),scene.dataset_type, multi_scale)
...
def training(model_param, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    ...
    gaussians = GaussianModel(model_param.sh_degree, hyper) #gaussian model initialize
    model_param.model_path = args.model_path
    timer = Timer()
    scene = Scene(model_param, gaussians, load_coarse=None) # scene initialize
    timer.start()
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, stage,ast_noise, dataset_type, multi_scale):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians,ast_noise=ast_noise,stage=stage, cam_type=dataset_type, *renderArgs, multi_scale=multi_scale)["render"], 0.0, 1.0)
