...
def render(viewpoint_camera, gs:GaussianModel, pipe, bg_color: torch.Tensor, ast_noise=None,scaling_modifier = 1.0, \
    ...
    if stage == "coarse" :
        ...
        if ast_noise is not None:
            time = time + ast_noise
            # print("[DEBUG]AST_NOISE, time",ast_noise[0],time[0]*10)
    ...
    rendered_image, radii,  depth, weight, _ = rasterizer(
    ...
    normal_map, rad_norm, dep_norm, weight_norm, _ = rasterizer(
        means3D = means3D_final,
        means2D = means2D,
        shs = normal_shs,
        opacities = opacity_final,
