...
class ModelHiddenParams(ParamGroup):
    def __init__(self, parser):
        ...
        # self.timenet_width = 32 # if you wanna eval deform-gpt
        self.timenet_output = 32
        # self.timenet_output = 4
        self.bounds = 1.6
        ...
        # self.empty_voxel=True
        self.grid_pe=0
        self.static_mlp=False
        # self.static_mlp=True
...
def get_combined_args(parser : ArgumentParser):
    ...
    # strange things happened: "config=<class 'arguments.config'>" is unnormal
    dstr = "config=<class 'arguments.config'>, "
    idx = cfgfile_string.find(dstr)
    # cfgfile_string.replace("config=<class 'arguments.config'>, ")
    if idx != -1:
        newstr = cfgfile_string[:idx] + cfgfile_string[idx+len(dstr):]
        # print(newstr)
        args_cfgfile = eval(newstr)
    else:
