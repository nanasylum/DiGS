...
import copy
@torch.no_grad()
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background,ast_noise, stage, iteration, time_now, dataset_type, multi_scale):
    def render(gaussians, viewpoint, path, scaling, cam_type):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(viewpoint, gaussians, pipe, background, ast_noise, stage=stage, cam_type=cam_type, multi_scale=multi_scale)
